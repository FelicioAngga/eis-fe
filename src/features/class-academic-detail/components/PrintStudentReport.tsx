...
function PrintStudentReport() {
  ...
  useEffect(() => {
    ...
    const addHeader = (doc: jsPDF) => {
      ...
      const lines = [
        { label: 'Nama', value: `${reportData?.data?.name}`, align: "left" },
        { label: 'NIS/NISN', value: `${reportData?.data?.nis}/${reportData?.data?.nisn}`, align: "left" },
        { label: 'Nama Sekolah', value: `${reportData?.data?.level} LETJEN HARYONO M.T.`, align: "left" },
        { label: 'Alamat', value: 'Jl. Pinang Baris II Gg Sekata', align: "left" },
        { label: 'Kelas', value: `${reportData?.data?.class}`, align: "right" },
        { label: 'Fase', value: `${reportData?.data?.fase}`, align: "right" },
        { label: 'Semester', value: `${reportData?.data?.term}`, align: "right" },
    ...
    const getPage2 = (doc: jsPDF) => {
      ...
      autoTable(doc, {
        ...
        body: [
          ["", "Sakit", ":", `${reportData?.data?.sick || '-'}`, "Hari"],
          ["", "Izin", ":", `${reportData?.data?.permission || '-'}`, "Hari"],
          ["", "Tanpa Keterangan", ":", `${reportData?.data?.absent || '-'}`, "Hari"],
        ],
        styles: {
          fontSize: 9,
          cellPadding: 1.5,
      ...
      doc.text(`Kepala ${reportData?.data?.level}`, 150, ttdY + 6);
      
      doc.text(`${reportData?.data?.home_room_teacher}`, 90, ttdY + 29);
      doc.text(`${reportData?.data?.principal}`, 150, ttdY + 29);
